...
def generate_words(graph) -> list[list[str]]:
    ...
    for curr_column in graph.nodes:
        for curr_node in curr_column:  # make words from every letter
            ...
            recurse(curr_node, curr_string, curr_path, possible_words, accepted_words, pattern,
                                          min_length, max_length)
            # for idx in range(len(possible_words)):
            #    possible_words[idx].extend(possible_words[idx])
    ...
    for direction in curr_node.directions.values():
        if direction:
            if direction not in curr_path:
                ...
                recurse(direction, new_string, curr_path, possible_words, accepted_words, pattern, min_length, max_length)
...
def game():
    alt_tab()
    g_input = process_board.get_board()
    alt_tab()
...
if __name__ == '__main__':
    ...
    # g = Graph(test_input)
    # print(g.to_string())
    # print(g.nodes[4][4].visual())
    # print(g.nodes[4][4].directions["up_right"].directions["down_right"].visual())
    start_time = time.time()
    game()
    print(f"Time: {time.time() - start_time}")
    # x = Thread(target=gethi)
    # print(x)
